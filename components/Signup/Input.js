    ...
    <div className="flex justify-center items-start flex-col font-lexend w-full my-4">
      ...
      <div className="rounded-full border-2 bg-white border-black px-4 py-2 text-2xl w-full flex">
        <input
          name={name}
          type={
            name === "password" || name === "confirmPassword" ? password : type
          }
        ...
        />
        {(name === "password" || name === "confirmPassword") && toggle && (
          <RxEyeNone onClick={() => handlePassword(false)} />
        )}
        {(name === "password" || name === "confirmPassword") && !toggle && (
          <RxEyeClosed onClick={() => handlePassword(true)} />
        )}
      </div>
    </div>
  );
